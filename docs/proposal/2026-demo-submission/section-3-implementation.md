...
| W4 | **Evidence Tier + 6-Layer Guardrail** | MedConf S/M/C 태깅+T1/T4 계층+금칙어+DUR 커버리지+종결+NLI(≥0.75)+의도 분류기(≥0.70) | NLI 한국어 의료 튜닝, paraphrase 우회 차단, Critic drop 임계값 | 상훈+주현 |
...
| **M2 데모 제출(예선)** | W7-W12 | W3 Critic 노드 + W4 Evidence Tier + NLI·의도 분류기 + W5 Langfuse+RAGAS+gold 200+red-team 10 + HIRA DUR 8종 + 제안서 5p + 3분 영상 | Critic 품질 튜닝, NLI 임계값, 영상 일정 |
...
**③ Data / Retrieval / Safety** — SQLite + FTS5 trigram · rapidfuzz 3.14.5 · 성분 동의어 사전 · DeBERTa-v3-xsmall NLI (ONNX 80MB) · KURE-v1 의도 분류기 · [데모] 식약처 3종 + HIRA DUR 8종 + KAERS · [결선] + RxNorm RRF + DailyMed SPL + ATC 브리지
