...
| **선택된 아키텍처 Profile** | **β (Grounded Scientist)** — Deterministic DUR + Evidence Tier tagging(MedConf) + LLM-as-judge critic(AMIE) + 5-Layer Guardrail |
...
| KURE-v1 임베딩 (Ko-MTEB 2025 SOTA) | 제품화 | 의도 분류기용, HF Hub nlpai-lab/KURE-v1 |
...
| **필케어 (본 제안)** | **소비자** | **식약처 3종 + HIRA DUR 8종 + KAERS(데모) / +RxNorm·DailyMed(결선)** | ✅ **결정론 N×N 성분쌍 + HIRA DUR 8종 룰 완전 적용** | ✅ **LangGraph 6-node (Deterministic 4 + LLM 2)** | **정보 안내 + DUR 경고 + 의료진 확인 경로 + 문서화** | ✅ **Evidence Tier + Critic + 5-layer guardrail (KURE-v1 의도 분류기)** |
...
│  │   3. DUR 커버리지 검증 ([CRITICAL] — 재생성 트리거)                    │   │
│  │   4. 필수 종결 문구 "의사 또는 약사와 상담하십시오"                    │   │
│  │   5. 의도 분류기 (KURE-v1 임베딩 유사도 ≥0.7, paraphrase 우회 차단)    │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ ⑤ 사용자 & 배포                                                        │   │
...
- **④ 안전 경계 5층**: §1.7⑤ 차별점을 시스템 구조로 가시화하며, 금칙어 단어 필터의 paraphrase 우회(Lakera 2024 벤치 60-90%)에 KURE-v1 의도 분류기로 대응한다.
...
| **M5** | **5-Layer Guardrail + 평가 Harness** | 금칙어 regex · 출처 계층 강제 · DUR 커버리지([CRITICAL]) · 필수 종결 · 의도 분류기(KURE-v1 임베딩 유사도 ≥0.7, fail-open) + Langfuse trace + RAGAS faithfulness/context-precision + 600 케이스 한국어 복약 gold set | 생성 응답 → 통과 / CRITICAL 재시도 / 의료진 escalate |
...
**🔹 D. 5-Layer Guardrail + KURE-v1 Intent Classifier**

시스템 · 모델 · 응답 3축에 걸친 5개 방어층 — 금칙어 regex + 출처 계층 강제(T1/T4) + DUR 커버리지([CRITICAL] 재생성) + 필수 종결 문구 + KURE-v1 임베딩 기반 의도 분류기(≥0.70, fail-open) — 를 배치한다. Lakera 2024 벤치가 보고한 단어 필터의 paraphrase 우회 60-90%에 대해 의미 수준 유사도 게이트로 대응한다. MedHallu(EMNLP 2025) correctness-vs-faithfulness 분리 주장에 부합하여 본 guardrail은 faithfulness(출처 근거 유지)를 강제하고 correctness(의학적 사실성)는 Critic 노드(10% 샘플)의 재검증에 위임한다. NLI entailment 게이트는 drug-scoped retrieval 인프라가 선행돼야 correctness 보장이 가능하다는 구조적 한계로 본 v2 데모에서 제외했으며 결선 단계 재도입 후보로 유지한다.
...
Pandit 외 MedHallu 벤치마크(10,000 QA)에서 GPT-4o · Llama-3.1 · UltraMedical 모두 의료 hard tier F1 **0.625**에 그친다 [arXiv:2502.14302]. 동 논문은 "not sure" 옵션 도입 시 최대 +38% 개선을 제시. 필케어는 이 공백을 **Evidence Tier tagging(MedConf S/M/C) + Critic drop + KURE-v1 의도 분류기** 3중 시스템 수준으로 대응한다.
...
Correctness-vs-Faithfulness 연구(arXiv:2412.18004, 2024-12)는 LLM 인용의 **최대 57%가 사후 합리화**임을 정량 입증했다. 필케어는 **Evidence Tier (MedConf S/M/C) + 독립 Critic (AMIE-style, 10% 샘플) + KURE-v1 의도 분류기** 3축으로 claim이 retrieved evidence에서 유래했는지 · 승인 경계를 넘지 않는지를 검증하며, 출처 태그 · DUR 커버리지 · 종결 문구 강제를 응답 스키마에 내장한다.
...
식약처 '생성형 AI 의료기기 허가·심사 가이드라인(2025-01-24)' 및 '디지털의료기기 분류 가이드라인(2025-05-07)' 기준 비의료기기(웰니스) 영역에서 운영하려면 판단 · 진단 · 처방 배제가 필수다. 단순 단어 replace 필터는 paraphrase 우회(Lakera 2024 벤치 60-90% 성공률)에 취약하다. 필케어는 **regex 금칙어 + 출처 계층(T1/T4) + DUR 커버리지([CRITICAL]) + 필수 종결 문구 + KURE-v1 임베딩 유사도 의도 분류기** 5계층에 Critic 의미 검증(10% 샘플)을 더해 "판단하지 않는다" 원칙을 의미 수준에서 방어하며, 약사법 §24 복약지도 경계를 유지한다.
...
| **W4** | **Evidence Tier + 5-Layer Guardrail** | MedConf Supported/Missing/Contradictory 태깅 + T1/T4 출처 계층 + 금칙어 regex + DUR 커버리지([CRITICAL]) + 필수 종결 + KURE-v1 의도 분류기(임베딩 유사도 ≥0.70, fail-open) | paraphrase 우회 차단, Critic drop 임계값, DUR 커버리지 재시도 루프 | 상훈 + 주현 |
...
| **M2 · 데모 제출(예선)** | W7-W12 | W3 Critic 노드 추가(6-node 완성) · W4 MedConf Evidence Tier + 5-Layer Guardrail(KURE-v1 의도 분류기) · W5 Langfuse + RAGAS + gold set 200 케이스 + red-team 10 · HIRA DUR 8종 완성 · 제안서 5p + 3분 영상 | Critic 품질 튜닝, 의도 분류기 임계값, 영상 촬영 일정 |
...
- KURE-v1 의도 분류기 (HF nlpai-lab/KURE-v1, HF 캐시 ~400MB, fail-open)
...
4. **구현 · 리서치 딥다이브** (사용자 요청 플래그):
   ...
   - KURE-v1 의도 분류기 임계값 튜닝 · forbidden-intent 풀 확장
...
| 2026-04-17 | **DeBERTa-v3 NLI entailment gate 제거 → 5-Layer Guardrail로 단순화** | A6 초기 구현의 NLI 게이트는 evidence_chunks를 drug 경계 없이 pooling하여 A-B 교차 지지 문제(P1 리뷰) 확인. drug-scoped retrieval 인프라가 선행돼야 correctness 보장 가능하므로 v2 데모 범위 외로 이연, 결선 재도입 후보. KURE-v1 의도 분류기(Layer 5)는 paraphrase-bypass 방어 용도로 유지(fail-open). DUR 커버리지 severity를 [ERROR] → [CRITICAL]로 승격해 LLM drop 발생 시 재생성 루프 작동 보장 |
