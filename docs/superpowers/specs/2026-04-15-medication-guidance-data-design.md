...
> **v2 업데이트 (2026-04-16)**: LangGraph StateGraph 오케스트레이션 + 4-Phase 매칭 + multi-ingredient DUR + 5-check guardrails.
> 상세 아키텍처: `specs/2026-04-16-langgraph-agent-architecture.md` 참조.

```
Stage 1. 입력 파싱 + 매칭 (Deterministic, LangGraph: match_drugs 노드)
│ 개인투약이력 (xls) → 약물명/EDI코드 추출
│ → 4-Phase 매칭: EDI 코드 → item_name 정확 → FTS5 trigram → rapidfuzz
│ → item_seq 확보 → 성분코드 복수 추출 (복합제 대응)
│ → 진료과/투약일수 메타데이터 보존
▼
Stage 2. DUR 전수 체크 (Deterministic, T1, LangGraph: check_dur 노드)
│ N개 약물의 모든 성분 조합 → dur_pairs lookup
│ → 병용금기 쌍 + 금기사유 확정
│ → 다기관(가정의학과 × 안과) 교차 쌍 플래그
▼
Stage 3. 약물별 정보 수집 (DB Lookup, T1, LangGraph: collect_info 노드)
│ item_seq별: drug_sections, drugs_easy, drugs 테이블에서 fetch
│ → 복약지도 10개 항목에 필요한 데이터 구조화
▼
Stage 4. Claude 생성 (LLM, LangGraph: generate 노드)
│ langchain-anthropic (ChatAnthropic) 약물별 순차 호출
│ System prompt: 복약지도 체크리스트 + 출처 티어 규칙 + 가드레일
│ Input: DUR 금기(T1) + 약물별 섹션(T1) + 환자 메타
│ → 별첨1(상세) + 병용금기 경고 + 별첨2(요약) + 별첨3(경고라벨)
│ → 모든 문장에 출처 태깅 [T1:허가정보] [T1:DUR] [T1:e약은요] [T4:AI]
▼
Stage 5. 사후 검증 (Deterministic, LangGraph: verify 노드)
│ 5-check guardrails:
│   1. DUR 커버리지 (구조화된 필드 매칭)
│   2. 출처 태그 존재 검증
│   3. T4 비율 ≤ 30% 제한
│   4. 필수 종결 문구 ("의사 또는 약사와 상담하십시오")
│   5. 금칙어 필터
│ → CRITICAL 실패 시 Stage 4 재시도 (max 1회)
```

### 5-1. Stage 1 상세: 약물명 매칭

투약이력의 약물명은 처방전 표기(브랜드명 + 함량 + 제형)로 되어 있음.
매칭 전략 (4-Phase, v2 업데이트):
1. **EDI 코드 정확 매칭**: 투약이력의 약품코드(drug_code) → drugs.edi_code 인덱스 (48% 커버리지, 100% 정확)
2. **item_name 정확 매칭**: drugs.item_name 인덱스
3. **FTS5 trigram 검색**: drugs_fts MATCH → top-5 후보 → rapidfuzz 재순위 (한국어 부분 매칭 지원)
4. **rapidfuzz 전수 스캔**: token_set_ratio ≥ 70 + 성분명(main_item_ingr) 기반 재매칭
5. 최종 실패 → 사용자에게 확인 요청

**성분 코드 추출** (v2 신규): MAIN_ITEM_INGR 필드에서 정규식 `\[([A-Z]\d+)\]`로 복수 추출.
복합제(예: 코대원정)는 `["M175201", "M146801"]` 같이 2+ 성분 코드를 반환.

**프로덕션 확장**: SapBERT-KO-EN 임베딩 매칭 (한영 혼합 의료 텍스트 특화, `huggingface.co/snumin44/sap-bert-ko-en`)
...
## 6. LangGraph 파이프라인 노드 정의

> **v2 업데이트 (2026-04-16)**: 기존 LLM tool-calling 방식에서 **LangGraph StateGraph 노드 방식**으로 변경. Deterministic 노드(Stage 1-3, 5)는 Python 함수, LLM 노드(Stage 4)만 Claude 호출. MedAgentBoard (NeurIPS 2025) 연구에 따라 single agent + 결정적 tool 조합 선택.

| 노드 | 타입 | 입력 State 키 | 출력 State 키 | 설명 |
|---|---|---|---|---|
| `match_drugs` | Deterministic | raw_records, db_path | matched_drugs, errors | 4-Phase 매칭 + 성분코드 추출 |
| `check_dur` | Deterministic | matched_drugs, db_path | dur_alerts | Multi-ingredient N×N DUR 교차 체크 |
| `collect_info` | Deterministic | matched_drugs, db_path | drug_infos | drugs + drug_sections + drugs_easy 조인 |
| `generate` | LLM (Claude) | drug_infos, dur_alerts, _llm | guidance_result | 약물별 순차 생성, 출처 태깅 |
| `verify` | Deterministic | guidance_result, dur_alerts | errors, _retry_count | 5-check guardrails |

### LangGraph DAG

```
match_drugs → check_dur → collect_info → generate → verify
                                            ↑           │
                                            └── retry ──┘ (CRITICAL + retry_count < 2)
                                                         │
                                                         └── done → END
```
...
- **모델**: Claude Sonnet 4.6 via `langchain-anthropic.ChatAnthropic`
- **호출 패턴**: 약물별 순차 호출 (11약물 = 11 API 호출, lost-in-the-middle 방지)
- **출처 grounding**: 프롬프트 기반 태그 강제 ([T1:허가정보], [T1:DUR], [T1:e약은요], [T4:AI])
- **프로덕션 경로**: Anthropic Citations API (char-level 인용 자동 추적)
- **비용 최적화**: langchain-anthropic 자동 prompt caching + token-efficient-tools beta
...
- **추가 데이터 소스** (딥리서치 2026-04-16 확인):
  - MFDS DUR 성분정보 API (`data.go.kr/data/15056780`): 연령금기, 임부금기, 노인주의 포함
  - HIRA DUR 의약품 목록 (`data.go.kr/data/15127983`, 2025-06 업데이트): 4개 카테고리 전체

### 7-4. DDI 데이터 보충 전략 (딥리서치 2026-04-16)

국제 무료 DDI 소스 현황:
- **DrugBank**: 무료 Interaction Checker **2026년 3월 종료 예정** → 장기 의존 불가
- **RxNav Interaction API**: **2024년 이미 종료** → 사용 불가
- **OpenFDA**: adverse event 보고 (구조화된 DDI 쌍 아님) → DDI 체크에 부적합
- **PrimeKG**: CC BY 4.0, 4M+ 관계 (indication/contraindication/DDI 포함) → ATC 브릿지로 보충 가능

**PillCare DDI 전략**: KIDS DUR (primary, 한국 규제 준수) + PrimeKG (supplementary, 국제 성분 커버리지)

### 7-5. 한국어 의료 NLP 모델 (딥리서치 2026-04-16)

약물명 매칭 및 향후 의미 검색에 활용 가능한 한국어 의료 모델:
- **SapBERT-KO-EN** (`snumin44/sap-bert-ko-en`): SNU 개발, KOSTOM 기반 한영 혼합 의료 텍스트 특화. 브랜드명/성분명/영문명 동시 매칭.
- **KM-BERT** (`madatnlp/km-bert`): 고려대 RIAS Lab, 한국어 의료 텍스트 600만 문장 학습. 의미 유사도 +0.046 Pearson 향상.
- **KBMC NER** (`SungJoo/medical-ner-koelectra`): 한국어 의료 NER (질환, 신체부위, 치료). LREC-COLING 2024.
- **BGE-M3** (BAAI): 다국어 8192토큰 dense+sparse+ColBERT. 한국어 의료 도메인 성능은 미검증 → 자체 Dev 셋 필요.
...
**금칙 어휘 리스트**: `진단합니다`, `처방합니다`, `투약판단`, `용량을 조절`, `복용을 중단하세요`, `복약지도`

**필수 종결 문구**: 주의사항/상호작용/투여종료후 섹션 끝에 "의사 또는 약사와 상담하십시오."

### 8-1. v2 Guardrail 체계 (5-check, 딥리서치 2026-04-16)

| # | 검증 | 방법 | 실패 시 |
|---|------|------|---------|
| 1 | DUR 커버리지 | GuidanceResult.dur_warnings에 모든 DUR 쌍 존재 확인 (구조화된 필드 매칭) | **CRITICAL** → 재생성 |
| 2 | 출처 태그 존재 | 모든 섹션에 `[T1:...]` 또는 `[T4:AI]` 정규식 매칭 | WARNING |
| 3 | T4 비율 제한 | T4 섹션 ≤ 전체 섹션의 30% | **CRITICAL** → 재생성 |
| 4 | 필수 종결 문구 | 주의사항/상호작용/투여종료후에 종결 문구 포함 | WARNING |
| 5 | 금칙어 필터 | 금칙 어휘 자동 제거 | WARNING (자동 수정) |

**재시도 정책**: CRITICAL 에러 발생 시 generate 노드로 루프백 (최대 1회). 2회 연속 CRITICAL 시 에러와 함께 결과 반환.

**프로덕션 확장** (딥리서치 근거):
- **NLI faithfulness 검증**: DeBERTa-v3 NLI로 T1 태그 문장이 원문과 entailment 관계인지 확인 (Self-RAG ICLR 2024 + Correctness-vs-Faithfulness arXiv:2412.18004 대응)
- **용량 환각 검증**: UD_DOC_DATA의 숫자 토큰과 생성 텍스트의 숫자 토큰 비교 (WARNING level)
- **Red-team suite**: 6개 카테고리 (자기진단 유도, 용량 변경 요청, cross-profile 누출, DUR 우회, 유사 약명 혼동, 규제 경계 넘기)
